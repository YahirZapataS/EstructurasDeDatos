...
int mejorAlumno(const vector<Alumno> &grupo)
{
    if (grupo.empty())
        return -1;
    ...
    {
        if (grupo[i].promedio > grupo[mAlumno].promedio)
        {
            mAlumno = i;
        }
    }
...
int main()
{
    ...
    do
    {
        ...
        case 3:
        {
            ...
            cin >> mat;
            int indice = buscarPorMatricula(grupo, mat);

            if (indice != -1)
            {
                cout << "\n--- Alumno Encontrado ---" << endl;
                cout << "Matricula: " << grupo[indice].matricula << endl;
                cout << "Nombre:    " << grupo[indice].nombre << endl;
                cout << "Promedio:  " << grupo[indice].promedio << endl;
            }
            else
            {
        ...
        case 6:
            ...
            cout << "\n--- Alumnos ordenados por promedio (Descendente) ---" << endl;
            mostrarAlumnos(grupo);
            break;

        case 7:
            ordenarPorPromedio(grupo, false);
            cout << "\n--- Alumnos ordenados por promedio (Ascendente) ---" << endl;
            mostrarAlumnos(grupo);
            break;
        case 8:
        ...
        case 9:
        {
            int mjrAlumno = mejorAlumno(grupo);
            if (mjrAlumno != -1)
            {
                cout << "\n=== El mejor alumno es: ===" << endl;
                cout << "Nombre: " << grupo[mjrAlumno].nombre << endl;
                cout << "Promedio: " << grupo[mjrAlumno].promedio << endl;
                cout << "Matricula: " << grupo[mjrAlumno].matricula << endl;
            }
            else
            {
                cout << "El grupo está vacío." << endl;
            }
            break;
        }
        }
